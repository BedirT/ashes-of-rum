        ...
        private IEnumerator Run()
        {
            ...
            try
            {
                ...
                archers = economy.DeployFriendlyForAutomation(FormationType.Archers,
                    new Vector3(-4f, 0f, 0.4f));
                archers.transform.rotation = Quaternion.Euler(0f, 8f, 0f);
                house = InstantiateRequired(HouseResource, new Vector3(6.2f, 0f, 5.1f));
                hisar = InstantiateRequired(HisarResource, new Vector3(-0.5f, 0f, 6.6f));

                foreach (var canvas in FindObjectsByType<Canvas>(FindObjectsSortMode.None))
                    canvas.gameObject.SetActive(false);
                HideUnrelatedRenderers(archers.transform, house.transform, hisar.transform);
                ...
                var target = new Vector3(0.3f, 1.15f, 4.1f);
                camera.transform.position = target + new Vector3(0.2f, 6f, -15f);
